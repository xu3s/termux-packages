termux_create_debian_subpackages() {
	...
	for subpackage in $TERMUX_PKG_BUILDER_DIR/*.subpackage.sh $TERMUX_PKG_TMPDIR/*subpackage.sh; do
		...
		[[ -n "$TERMUX_SUBPKG_BREAKS"           ]] && echo "Breaks: $TERMUX_SUBPKG_BREAKS"
		[[ -n "$TERMUX_SUBPKG_CONFLICTS"        ]] && echo "Conflicts: $TERMUX_SUBPKG_CONFLICTS"
		[[ -n "$TERMUX_SUBPKG_RECOMMENDS"       ]] && echo "Recommends: $TERMUX_SUBPKG_RECOMMENDS"
